...
public final class ClaimEditorSkeleton implements ClaimEditor {
    ...
    private @NotNull ClaimEditResult handleSelectSegment(
        @NotNull ClaimEditorSession session,
        @NotNull ClaimEditIntent intent
    ) {
    ...
    private @NotNull ClaimEditResult handleExpandSegment(
        @NotNull ClaimEditorSession session,
        @NotNull ClaimEditIntent intent
    ) {
        if (session.mode() != ClaimEditorMode.SHAPED) {
        ...
        if (!validationResult.isValid()) {
            ...
            ClaimEditFailureType failureType = validationResult
                .issues()
                .stream()
    ...
    private @NotNull ClaimEditResult handleAddCorner(
        @NotNull ClaimEditorSession session,
        @NotNull ClaimEditIntent intent
    ) {
        if (session.mode() != ClaimEditorMode.SHAPED) {
        ...
        if (!isOrthogonalStep(lastPoint, point)) {
            ...
            if (point.equals(lastPoint)) {
                ...
                ClaimEditPreview preview = draftPreview(
                    snappedDraft,
                    basePolygon,
                    Collections.singletonList("Shaped claims must turn on the same X or Z axis.")
                );
                return ClaimEditResult.failure(
                    ClaimEditFailureType.INVALID_GEOMETRY,
                    null,
        ...
        if (basePolygon != null && !reconnectCandidates.isEmpty() && reshapeLeavesBoundary) {
            ...
            if (bestMerge != null) {
                ShapedPathDraft updatedDraft = new ShapedPathDraft(
                    draft.claimId(),
                    bestMerge.candidatePoints(),
                    null,
                    true
                );
        ...
        if (!validationResult.isValid()) {
            ClaimEditPreview preview = previewFromValidation(
                ...
                basePolygon,
                validationResult,
                Collections.singletonList("This shaped claim would intersect itself or violate closure rules.")
            );
            ClaimEditFailureType failureType = validationResult
                .issues()
                .stream()
        ...
        ClaimEditPreview preview = closedPreview(
            ...
            rawPoint.equals(point)
                ? Collections.singletonList("Shaped path closed.")
                : Collections.singletonList("Shaped path snapped closed.")
        );
        return ClaimEditResult.success(
            session.withOpenPath(updatedDraft).withPreview(preview),
            preview,
    ...
    private static final class BoundaryMergeAttempt {
        ...
        @NotNull
        List<OrthogonalPoint2i> candidatePoints() {
    ...
    private @NotNull List<OrthogonalPoint2i> findBoundaryReconnectPoints(
        ...
        candidates.sort(
            Comparator.comparingInt(
                (OrthogonalPoint2i candidate) -> Math.abs(candidate.x() - from.x()) + Math.abs(candidate.z() - from.z())
            )
    ...
    private @Nullable BoundaryMergeAttempt selectBestBoundaryMerge(
        ...
        for (OrthogonalPoint2i reconnectPoint : reconnectCandidates) {
            ...
            if (best == null || compareBoundaryMergeAttempts(attempt, best) < 0) {
                best = attempt;
            }
        }
    ...
    private @Nullable OrthogonalPoint2i nearestIntersectionPoint(
        ...
        if (segment.isHorizontal() && boundary.isVertical()) {
            ...
            return segment.containsPoint(point) && boundary.containsPoint(point) && !point.equals(excludedPoint)
                ? point
                : null;
        }
    ...
    private @Nullable OrthogonalPoint2i nearestOverlapPoint(
        ...
        int step = horizontal ? Integer.compare(to.x(), from.x()) : Integer.compare(to.z(), from.z());
        if (step == 0) {
            return null;
        }
    ...
    private @NotNull OrthogonalPoint2i snapToOrthogonal(
        @NotNull OrthogonalPoint2i anchor,
        @NotNull OrthogonalPoint2i rawPoint
    ) {
    ...
    private @NotNull ClaimEditPreview previewFromValidation(
        ...
        List<OrthogonalPoint2i> conflictPoints = validationResult
            .issues()
            .stream()
    ...
    private @NotNull OrthogonalPolygonValidationResult mergeDraftIntoExistingPolygon(
        ...
        OrthogonalPolygonValidationResult unionFallback = tryUnionFallback(
            working,
            draftPoints,
            forwardCandidate,
            reverseCandidate
    ...
    private @NotNull List<OrthogonalPoint2i> pathAlongPolygon(
        ...
        while (index != toIndex) {
            index = forward ? (index + 1) % corners.size() : (index - 1 + corners.size()) % corners.size();
            path.add(corners.get(index));
        }
        return path;
    }
    ...
    private @NotNull OrthogonalPolygonValidationResult chooseMergedCandidate(
        ...
        if (!firstValid) {
            throw new IllegalArgumentException(
                "That reshape path cannot merge cleanly into the existing claim boundary."
            );
        }
        ...
        boolean extendsOutsideOriginal = draftPoints
            .stream()
            .skip(1)
    ...
    private @Nullable OrthogonalPolygonValidationResult tryUnionFallback(
        ...
        if (first.isValid() && first.polygon() != null && !polygonContainsPolygon(first.polygon(), originalPolygon)) {
            ...
            second.isValid() && second.polygon() != null && !polygonContainsPolygon(second.polygon(), originalPolygon)
        ) {
    ...
    private @NotNull ClaimEditResult handleAddNode(
        @NotNull ClaimEditorSession session,
        @NotNull ClaimEditIntent intent
    ) {
        if (session.mode() != ClaimEditorMode.SHAPED) {
        ...
        if (existingCornerIndex >= 0) {
            ...
            Integer mergedEdgeIndex = findMatchingEdgeIndex(
                updatedPolygon,
                polygon.corners().get(Math.floorMod(existingCornerIndex - 1, polygon.corners().size())),
                polygon.corners().get((existingCornerIndex + 1) % polygon.corners().size())
            );
            SegmentSelection selection =
                mergedEdgeIndex == null
                    ? null
    ...
    private @NotNull SegmentExpansion expandSegment(
        ...
        if (amount == 0) {
            return new SegmentExpansion(OrthogonalPolygon.validatePath(polygon.closedPath()), activeSegment);
        }
        ...
        debug(
            "expandSegment claim=%s edgeIndex=%d amount=%d edge=%s movedStart=%s movedEnd=%s polygon=%s",
        ...
        if (directResult.isValid() && directResult.polygon() != null) {
            ...
            debug(
                "expandSegment directResult polygon=%s directEdgeIndex=%s issues=%s",
                directResult.polygon().corners(),
                directEdgeIndex,
                directResult.issues()
            );
            SegmentSelection directSelection =
        ...
        OrthogonalPolygon sweep = OrthogonalPolygon.fromClosedPath(
            Arrays.asList(edge.start(), edge.end(), movedEnd, movedStart, edge.start())
        );
        debug("expandSegment sweep=%s", sweep.corners());

        OrthogonalPolygon updatedPolygon =
            signedAmount > 0 ? OrthogonalPolygon.union(polygon, sweep) : subtractPolygons(polygon, sweep);
        ...
        debug(
            "expandSegment updatedPolygon=%s updatedEdgeIndex=%s issues=%s",
            updatedPolygon.corners(),
            updatedEdgeIndex,
            result.issues()
        );
        SegmentSelection updatedSelection =
            updatedEdgeIndex == null
                ? null
    ...
    private @NotNull OffsetVector outwardOffset(@NotNull OrthogonalPolygon polygon, @NotNull OrthogonalEdge2i edge) {
        ...
        int leftDx = -dz;
        int leftDz = dx;
        return counterClockwise ? new OffsetVector(leftDx, leftDz) : new OffsetVector(dz, -dx);
    }
    ...
    private boolean isConnected(@NotNull Set<OrthogonalPoint2i> occupied) {
        ...
        for (int i = 0; i < queue.size(); i++) {
            ...
            for (OrthogonalPoint2i neighbor : Arrays.asList(
